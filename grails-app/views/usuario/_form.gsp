...
<div class="fieldcontain ${hasErrors(bean: usuarioInstance, field: 'medias', 'error')} ">
	...
	<g:select name="medias" from="${mycloudmedia.Media.list()}" multiple="multiple" optionKey="id" size="5" value="${usuarioInstance?.medias*.id}" class="many-to-many"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: usuarioInstance, field: 'subscription', 'error')} required">
	...
	<g:select id="subscription" name="subscription.id" from="${mycloudmedia.Subscription.list()}" optionKey="id" required="" value="${usuarioInstance?.subscription?.id}" class="many-to-one"/>
</div>
